...
const directoryPath = path.join(app.getAppPath(), 'jots');
...
const getJot = async (fileName: string) => {
  try {
    ...
    if (sanitizedFileName === 'index.json') return null;

    const file = await readFile(
      path.join(directoryPath, sanitizedFileName),
      'utf-8'
    );
...
const writeJot = async (jot: Jot) => {
  try {
    ...
    await writeFile(
      path.join(directoryPath, `${jot.id}.json`),
      JSON.stringify(jot),
      'utf-8'
    );
